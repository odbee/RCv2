...
class ImageSubscriber(Node):
  ...
  def ocv_callback(self,request, response):
    kp1,des1=self.detector.detectAndCompute(self.current_frame,None)
    ...
    try:
      if (max(bestList))>=12:
        ...
        self.which_object_msg.data=self.classNames[bestList.index(max(bestList))]
        self.get_logger().info('data: %s' %self.which_object_msg.data )
      else:
        self.which_object_msg.data=1000
    except:
      self.which_object_msg.data=1000
    ll= len(bestList)
    for i in range(len(bestList)):
      strin = self.classNames[i] + ' ' + str(bestList[i])
      cv2.putText(self.current_frame,strin,(50,50+30*i),cv2.FONT_HERSHEY_COMPLEX,0.7,(0,0,255),2,4)
    # if bestIndex != -1:
    ...
    # self.which_object_msg.data=self.classNames[bestIndex]
