...
from adam_msgs.action import FollowGripper
...
class GripperControlServer(Node):
    ...
    def execute_callback(self,goal_handle):
        ...
        feedback_msg=FollowGripper.Feedback()
        result = FollowGripper.Result()
        ...
        if (request.command=='a'):
            ...
            if (self._get_var(self.ACT)==1):
                set_ok,cmd_pos=self.move(request.position,request.speed,request.force)
                if not set_ok:
                    self.get_logger().info("Failed to set variables for move.")
                    result.returncode = 2
                while self._get_var(self.PRE) != cmd_pos:
                    time.sleep(0.001)
                cur_obj = self._get_var(self.OBJ)
                while GripperControlServer.ObjectStatus(cur_obj) == GripperControlServer.ObjectStatus.MOVING:
                    cur_obj = self._get_var(self.OBJ)
                    feedback_msg.currentposition=self.get_current_position()
                    self.get_logger().info('Publishing feedback: {0}'.format(feedback_msg.currentposition))
                    # Publish the feedback
                    goal_handle.publish_feedback(feedback_msg)
                    time.sleep(0.06)
                result.returncode = 1
            else:
                self.get_logger().info("Failed to move. please activate gripper")
                result.returncode=2
        ...
        result.returnpos=self.get_current_position()
        result.objectstatus=self._get_var(self.OBJ)
...
def main(args=None):
    ...
    gripper_control_server.connect('192.168.1.112',63352)
